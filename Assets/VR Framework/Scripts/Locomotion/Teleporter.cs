...
namespace VRFramework.Locomotion
{
    public class Teleporter : MonoBehaviour
    {
        ...
        private void OnTeleportPressed(VrInputActionData _data)
        {
            ...
            {
                Teleport(pointer.Endpoint);
            }
        }

        protected virtual bool ValidTeleportLocation(Vector3 _position)
        {
            if(!Enabled || _position == Vector3.zero)
                return false;

            bool validNavMeshLocation = false;
            // Check the navmesh to see if this is a location we can teleport to
            validNavMeshLocation = navMeshData == null || 
                NavMesh.SamplePosition(_position, out NavMeshHit _, navMeshData.distanceLimit, navMeshData.validAreas);
